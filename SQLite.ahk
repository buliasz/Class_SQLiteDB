...
; AHK version:      2.0.10
; Tested on:        Win 10 (x64), SQLite 3.11.1
...
class SQLiteClass {
   ...
   Exec(sql, Callback := "") {
      ...
      returnCode := DllCall("SQlite3.dll\sqlite3_exec", "Ptr", this._handle, "Ptr", &utf8, "Int", CBPtr, "Ptr", Object(this)
                  , "UPtrP", Err, "Cdecl Int")
      if (CBPtr) {
   ...
   ;                       &ST    - Variable to store the statement object (Class _Statement)
   ...
   CreateScalarFunc(Name, Args, Func, Enc := 0x0801, param := 0) {
      ...
      returnCode := DllCall("SQLite3.dll\sqlite3_create_function", "Ptr", this._handle, "AStr", Name, "Int", Args, "Int", Enc
   ...
   ;                                     address : Address of the (variable containing the) BLOB.
         ...
         loop returnCode {
            ...
            if (columnType == SQLITE_NULL) {
               ...
               if (blobPtr == 0) || (blobSize == 0) {
                  ...
                  address := row[A_Index].GetAddress("blob")
   ...
   __New() {
      ...
      if (this.base._refCount == 0) {
         ...
         dll := DllCall("LoadLibrary", "Str", this.base._sqliteDll, "UPtr")
         if (!dll) {
         ...
         if (sqlVersion[1] < minVersion[1]) || ((sqlVersion[1] == minVersion[1]) && (sqlVersion[2] < minVersion[2])){
            ...
            MsgBox("SQLite ERROR, Version " this.base.version " of SQLite3.dll is not supported!`n`n"
   ...
   __Delete() {
      ...
      this.base._refCount -= 1
      if (this.base._refCount == 0) {
         dll := DllCall("GetModuleHandle", "Str", this.base._sqliteDll, "UPtr")
         if (dll) {
            DllCall("FreeLibrary", "Ptr", dll)
         }
      }
